...
- **Autenticazione agenti**: registrazione e login via Supabase Auth (email + password), logout, protezione delle route tramite `src/proxy.ts` (Next.js 16) e verifica server-side. Alla registrazione l'account viene **creato già attivo** con la service role key (`admin.createUser` con `email_confirm: true`): **Supabase non invia nessuna email di conferma** e l'agente entra subito nella dashboard con le credenziali appena create (se la chiave non è configurata, l'app usa la registrazione standard che può richiedere la conferma via email).
...
- **Invio email ordini**: canale SMTP dell'account aziendale (default Aruba `smtps.aruba.it:465`, `ordinidetomaso@etnatobacco.com`) con fallback Resend. La password SMTP può essere impostata dalle **Impostazioni → Configurazione server email** (store cifrato AES-256-GCM) oppure come variabile d'ambiente `EMAIL_ACCOUNT_PASSWORD` (utile in produzione). Server/account/porte sono sovrascrivibili con `EMAIL_SMTP_SERVER`, `EMAIL_SMTP_PORT`, `EMAIL_SMTP_SECURE`, `EMAIL_ACCOUNT`, `EMAIL_USERNAME`, `EMAIL_DISPLAY_NAME`. Il destinatario è `ORDER_EMAIL_TO` (impostazione o env). Dalla pagina **Impostazioni** c'è il pulsante **"Invia email di prova"** (`/api/test-email`, solo amministratore).
- **Cartella dati robusta**: `data/` viene risolta in ordine da `DATA_DIR` (env) → `process.cwd()/data` → ricerca verso l'alto. L'app funziona anche se avviata con una working directory diversa dalla root del progetto.
- **Funzionamento online (Vercel/Supabase)**: su Vercel il filesystem è in sola lettura, quindi admin, provvigioni, spedizioni, sub-amministratori, override catalogo e stato lettura ordini vengono salvati nella tabella Supabase `app_settings`; i file Excel degli ordini e il template di lavoro del catalogo vengono salvati nel bucket Storage `ordini`. Segui **`VERCEL-SETUP.md`** per configurare migrazione, variabili d'ambiente e primo accesso online.
...
5. Avvia l'app:
   ...
   Apri http://localhost:3000. Crea la password amministratore e registra un agente:
   l'account viene attivato subito e l'agente entra direttamente nella dashboard
